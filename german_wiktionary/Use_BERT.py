...
def process_number(relevant_pairs,entry_dict,model,tokenizer,frame):
    for (singular,plural) in tqdm(relevant_pairs):
        ...
        gender_sg = entry_dict[singular]["gender"]
        gender_pl = entry_dict[plural]["gender"]
        sg_flections = entry_dict[singular]["flections"]
        pl_flections = entry_dict[plural]["flections"]
        for w_index in word_senses.keys():
            ...
            for example in word_examples[w_index]:
                ...
                if singular in marked_text.split():
                    ...
                    continue
                frame = frame.append({"Word":singular,"Number":"Sg","Gender":gender_sg,"Sense":sense,"Sentence":example,"Word Vector": word_vector, "Sentence Vector": sentence_embedding},ignore_index=True)
        for p_index in plural_senses.keys():
            ...
            for example in plural_examples[p_index]:
                ...
                if plural in marked_text.split():
                    ...
                    continue
                frame = frame.append({"Word":plural,"Number":"Pl","Gender":gender_pl,"Sense":sense,"Sentence":example,"Word Vector": word_vector, "Sentence Vector": sentence_embedding},ignore_index=True)
    return frame
...
def process_other(other,entry_dict,model,tokenizer,frame):
    for item in tqdm(other):
        ...
        gender = entry_dict[item]["gender"]
        flections = entry_dict[item]["flections"]
        for index in senses.keys():
            ...
            for example in examples[index]:
                ...
                if item in marked_text.split():
                    ...
                    continue
                frame = frame.append({"Word":item,"Gender":gender,"Sense":sense,"Sentence":example,"Word Vector":word_vector,"Sentence Vector":sentence_embedding},ignore_index=True)
    return frame

def write_files(filename,tokenizer,model,mode):
    entry_dict = read_files(filename)
    number_pairs,gender_list, other = find_pairs(entry_dict)
    frame = pd.DataFrame(columns=["Word", "Number", "Gender", "Sense", "Sentence", "Word Vector", "Sentence Vector"])
    if mode == "-n": # for number
        frame = process_number(number_pairs,entry_dict,model,tokenizer,frame)
        frame.to_csv("number.csv",sep="\t")
        frame.to_pickle("number.pkl")
    elif mode == "-g": # for gender
        frame = process_gender(gender_list,entry_dict,model,tokenizer,frame)
